...
Take off 4 points if they do pnorm in r and do not do the z score by hand. (i.e print("Y∼N(1,4). Find P(Y ≤ 3).")
print(pnorm(q=3,mean=1,sd=sqrt(4))))
...
### Question 2: 20 points total, 10 points each 
if students do the 0 and 1 the other way around that is fine, give full marks. Variance will be the same, phat will be 1 - phat, confidence interval will be 1 - c.i. on the answer key 
...
phat = 215/400
s2 = phat * (1-phat)
lower = phat-1.96*sqrt(s2/400)  
upper = phat+1.96*sqrt(s2/400) 
```

a. $$\hat{p} = 215/400 = 0.5375$$
   $$S^2_p = 0.5375*(1-0.5375) = 0.2486$$
...
a.
$$E(\bar{Y}) =8 $$
$$SE_Y = S^2_Y / \sqrt{n} = 4/27 $$
$$P(\bar{Y}<10) = \Phi( \frac{10-8}{\sqrt{4/27}} ) = \Phi(5.19)$$
```{r,echo=F}
pnorm(2/sqrt(4/27))
```
which equals to 0.99999 (or approximately 1).
...
# calculate the proportion that the sample mean is less than 10. 
```
The empirical probability that $\bar{Y}<10$ is `r p`.

### Question 4: 5 points each, 30 total 
...
a. Plot a histogram of median owner-occupied housing value in the area. 
b. Calculate the mean, median, minimum, and maximum housing value in the Boston area
c. Are housing values adjacent the Charles River higher? Explain. 
d. Plot the bivariate relationship between the property tax rate and the pupil-teacher ratio. Are they correlated?
e. Create an indicator variable (taking the value of 0 and 1) showing whether the tract is above or below median crime rate of the city. What is the conditional variance of median housing values for tracts above the median crime rate? Below the median crime rate?
...
## e.
First, create an indicator variable. There are multiple ways to do this: demonstration through the mutate function:
```{r}
...
Formally, this can be done using an F-test, which rejects the null hypothesis at 95% confidence that the two variances are not the same. 
